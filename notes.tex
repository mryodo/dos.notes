...
Let us assume that \( \sigma \left(  A  \right) \subset [-1, 1] \); otherwise, one can reschedule the operator such that the spectrum lands inside \( [-1, 1]\) segment (e.g. by \( A \to \frac{2}{\lambda_{\max}} A - I \))\todo{here we would need to compute at least the leading eigenvalue, but this is relatively cheap and stable, right? Right?}.
